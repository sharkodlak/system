# Encrypted disk and HOME
apt-get install -y cryptsetup ecryptfs-utils

# Install system monitor wia Chrome
apt-get install -y chrome-gnome-shell #gir1.2-gtop-2.0 gir1.2-networkmanager-1.0 gir1.2-clutter-1.0
# Visit https://extensions.gnome.org/extension/945/cpu-power-manager/
# Visit https://extensions.gnome.org/extension/120/system-monitor/

# Work
apt-get install -y git gitk php-cli vagrant virtualbox virtualbox-guest-additions-iso
snap install atom --classic

# Browsers
snap install opera

# Net
apt-get install -y wakeonlan
